...
exports.verifyWhatsappPage = [
  ...
  async (req, res) => {
    const user = await User.findById(req.user.id)
      .select('whatsappVerifiedAt whatsappLocalNumber whatsappTokenExpires')
      .lean();
    ...
    res.render('webview/verify-whatsapp', {
      ...
      currentNumber: (user && user.whatsappLocalNumber) || '',
      pending,
...
exports.sendWhatsappCode = [
  ...
  async (req, res) => {
    try {
      ...
      const parsed = wa.normalizeNigerian(req.body.phone);
      if (!parsed) {
        return res.json({ success: false, message: 'Enter a valid Nigerian mobile number, e.g. 0803 123 4567.' });
      }
      ...
      const masked = parsed.local.slice(0, 4) + '****' + parsed.local.slice(-3);
...
exports.verifyWhatsappCode = [
  ...
  async (req, res) => {
    try {
      ...
      logger.info(`[WHATSAPP] ${user.email} verified ${user.whatsappLocalNumber}`);
      return res.json({
        success: true,
